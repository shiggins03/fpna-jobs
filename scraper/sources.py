...
UA = {"User-Agent": "fpna-jobs-aggregator (personal job search; contact via repo)"}
TIMEOUT = 30
...
def fetch_google_careers(co, query):
    # The endpoint carries the query in its URL, so the configured url is a
    # template with a {query} placeholder (unifier-jobs called this adapter
    # with a fixed single-keyword URL).
    url = co["url"].replace("{query}", quote_plus(query))
    try:
